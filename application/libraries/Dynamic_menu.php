...
class Dynamic_menu {
     ...
     */
    function build_menu($table = 'menu')
    {
        $menu = array();
            ...
            if (is_array($menu[$i]))    // must be by construction but let's keep the errors home
            {
                if ($menu[$i]['show'] && $menu[$i]['parent'] == 0)    // are we allowed to see this menu?
                {
                    if ($menu[$i]['is_parent'] == TRUE)
                    {
                        ...
                        $html_out .= "\t\t\t".'<li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">'.$menu[$i]['title'].'</a>';
                        //.anchor('#', '<span>'.$menu[$i]['title'].'</span>');
                    }
                    else
                    {
                        $html_out .= "\t\t\t\t".'<li class="dropdown"><a href="'.$menu[$i]['url'].'" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">'.$menu[$i]['title'].'</a>';
                    }
       ...
       // $html_out .= "\n\t\t\t\t".'<div>'."\n";
        ...
        for ($i = 1; $i <= count($menu); $i++)
        {
            if ($menu[$i]['show'] && $menu[$i]['parent'] == $parent_id)    // are we allowed to see this menu?
            {
                ...
                if ($menu[$i]['is_parent'] == TRUE)
                {
                    $html_out .= "\t\t\t".'<li class="menu-item dropdown dropdown-submenu"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">'.$menu[$i]['title'].'</a>';
                }
                else
                {
